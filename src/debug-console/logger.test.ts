...
test('returns file removed snapshot message', () => {
  expect(logger(' › snapshot file removed.')).toMatchInlineSnapshot(
    `"[1m[32m › snapshot file removed.[0m[0m"`,
  )
})

test('returns removed snapshot message', () => {
  expect(logger(' › 1 snapshot removed.')).toMatchInlineSnapshot(
    `"[1m[32m › 1 snapshot removed.[0m[0m"`,
  )
})

test('returns obsolete snapshot message', () => {
  expect(logger(' › 1 snapshot obsolete.')).toMatchInlineSnapshot(
    `"[1m[33m › 1 snapshot obsolete.[0m[0m"`,
  )
})
...
test('returns file removed snapshot summary (sigular)', () => {
  expect(
    logger(' › 1 snapshot file removed from 1 test suite.'),
  ).toMatchInlineSnapshot(`" [1m[32m› 1 snapshot file removed[0m[0m from 1 test suite."`)
})

test('returns files removed snapshot summary (plural)', () => {
  expect(
    logger(' › 2 snapshot files removed from 2 test suites.'),
  ).toMatchInlineSnapshot(`" [1m[32m› 2 snapshot files removed[0m[0m from 2 test suites."`)
})

test('returns removed snapshot summary (sigular)', () => {
  expect(
    logger(' › 1 snapshot removed from 1 test suite.'),
  ).toMatchInlineSnapshot(`" [1m[32m› 1 snapshot removed[0m[0m from 1 test suite."`)
})

test('returns removed snapshot summary (plural)', () => {
  expect(
    logger(' › 2 snapshots removed from 2 test suites.'),
  ).toMatchInlineSnapshot(`" [1m[32m› 2 snapshots removed[0m[0m from 2 test suites."`)
})
...
test('returns obsolete snapshot summary (sigular)', () => {
  expect(
    ...
    `" [1m[33m› 1 snapshot obsolete[0m[0m from 1 test suite. [90mTo remove them all, re-run jest with \`JEST_TEST_RUNNER_UPDATE_SNAPSHOTS=true\`.[0m"`,
  )
})
...
test('returns obsolete snapshot summary (plural)', () => {
  expect(
    logger(
      ' › 2 snapshots obsolete from 2 test suites. To remove it, re-run jest with `-u`.',
    ),
  ).toMatchInlineSnapshot(
    `" [1m[33m› 2 snapshots obsolete[0m[0m from 2 test suites. [90mTo remove them all, re-run jest with \`JEST_TEST_RUNNER_UPDATE_SNAPSHOTS=true\`.[0m"`,
  )
})

test('returns obsolete test path', () => {
  expect(logger('   ↳ src/path/to/file.test.ts')).toMatchInlineSnapshot(
    `"   ↳ [90msrc/path/to/[0m[1mfile.test.ts[0m"`,
  )
})
...
test('returns failed snapshot summary (sigular)', () => {
  expect(
    ...
    `" [1m[31m› 1 snapshot failed[0m[0m from 1 test suite. [90mInspect your code changes or re-run jest with \`JEST_TEST_RUNNER_UPDATE_SNAPSHOTS=true\` to update them.[0m"`,
  )
})
...
test('returns failed snapshot summary (plural)', () => {
  expect(
    ...
    `" [1m[31m› 2 snapshots failed[0m[0m from 2 test suites. [90mInspect your code changes or re-run jest with \`JEST_TEST_RUNNER_UPDATE_SNAPSHOTS=true\` to update them.[0m"`,
  )
})

// test('returns failed snapshot summary (plural) 123', () => {
//   expect(
//     logger(
//       ' › 2 snapshots failed from 2 test suites. Inspect your code changes or run `yarn test -u` to update them.',
//     ),
...
// test('returns failed snapshot summary (plural) 456', () => {
//   expect(
//     logger(
//       ' › 2 snapshots failed from 2 test suites. Inspect your code changes or run `yarn test -u` to update them.',
//     ),
